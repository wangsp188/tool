...
/**
 * @Description 二进制字符串表达状态码(最多32个) 可自行添加在执行器中打标即可
 * @Author wangshaopeng
 * @Date 2020-07-15
 */
public enum Status {
	valid("是否验证通过,验证通过才会执行", 0), async("是否是异步执行", 1), normalEnd("是否正常结束", 2), abortEnd("是否非正常结束(流程代码执行非正常结束)", 3), timeout("simpleProcess使用,是否超时", 4), rollback("simpleProcess使用,是否发生回滚", 5), interrupt("simpleProcess使用,是否执行中断(doChain未执行)", 6),;
