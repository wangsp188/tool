...
public @interface Excel {
	...
	/**
	 * 字典替换 形似{"1::男","0::女"} key::val ::分隔 通用 字典
	 */
	 ...
	 * 图片保存策略
	 * 
	 * @see ImgStoreStrategyFactory and ImgStore 通用属性
	 */
	 ...
	 * 图片构建策略
	 */
	ImgProduceStrategy imgProduceStrategy() default ImgProduceStrategy.adaptable;
	...
	/**
	 * 属性为空时构建的单元格显示 构建时,该属性是null时的单元格值设置
	 */
	 ...
	 * 或者是静态函数 eg cn.wang.A.test 横版和模板解析都用作属性的自定义设置
	 *
	 * @return
	 */
	String methodParseConvert() default "";

	/**
	 * 构建时转换函数 pojo中加入函数 返回值 CellData 值(Object obj);
	 * 横版和模板构建都用作属性的自定义构建(如果该属性是集合类型,传入的值是集合里的单个数据) 或者是静态函数 eg cn.wang.A.test2
	 * 
	 * @return
	 */
	String methodProduceConvert() default "";
}
