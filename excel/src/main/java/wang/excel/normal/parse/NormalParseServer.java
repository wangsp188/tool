...
public class NormalParseServer {
	...
	private ParseResult parseSheet(Sheet sheet, ParseParam param) {
		...
		try {
			...
			// 解析参数
			List<ColParseParam> params = parseTitles(cells, param.getCol2Field(), param.getNotNullArr());
			...
			while (parseOne.has() && maxParse > currentNum && !Thread.interrupted()) {
	 ...
	 */
	private void modifyFieldConverter(List<ColParseParam> params, Map<String, ParseConvert> importConvertMap) {
		if (importConvertMap != null) {
			for (ColParseParam param : params) {
	...
	private List<ColParseParam> parseTitles(Collection<Cell> cells, Col2Field col2Field, String[] notNullArr) throws Exception {

		List<ColParseParam> titles = new ArrayList<>();
		...
		for (Cell cell : cells) {
			...
			titles.add(new ColParseParam(cell));
		}
		// 解析字段和表头的对应关系
	 ...
	 */
	private void initTitle2Field(List<ColParseParam> cpL, Col2Field col2Field, String[] notNullArr) throws Exception {
		for (ColParseParam cp : cpL) {
			...
			cp.setBeanInnerParseConvert(fp.getBeanInnerParseConvert());
